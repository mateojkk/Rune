...
### 2. Privacy-by-Default Encryption (Seal)
Rune is built on a "Privacy by Default" philosophy. All submissions are automatically protected using **Seal threshold encryption**. Submissions are encrypted on the client side before ever hitting the storage layer, ensuring that only the form owner can decrypt and view the results. Your data is private by design, not by choice.
...
2. **Automated Security:** All responses are automatically encrypted with Seal threshold encryption before being sent.
3. **Publish:** Set your form to `Published` and share the permanent URL.
4. **Collect:** Submissions are bundled and stored immutably on Walrus.
5. **Analyze:** Decrypt and view your private feedback directly from the dashboard.
